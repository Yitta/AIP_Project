...
export class AuthenticationService {
  ...
  getResetEmail(email) {
    return this.http.post(`api/auth/forgot`, email, { headers: this.headers })
      .map(res => {
        res.json();
      })
      .catch(this._errorHandler);
  ...
  resetPassword(passwordInfo) {
    return this.http.post(`api/auth/reset`, passwordInfo, { headers: this.headers })
      .map(res => res.json())
      .catch(this._errorHandler);
  }

  currentUser() {
    return this.http.get(`api/auth/current`, { headers: this.headers })
      .map(res => res.json())
      .catch(this._errorHandler);
  }
